...
=======
import { GoogleAuthProvider, GithubAuthProvider, signInWithPopup, signInWithEmailAndPassword, createUserWithEmailAndPassword, User, signInWithRedirect } from "firebase/auth";
>>>>>>> origin/main
...
export function GoogleSignInButton({
  ...
  const doSignInWithGoogle = async () => {
    ...
    try {
      ...
      if (resp && resp.user) {
        ...
        if (userExists && onExistingUser) {
          onExistingUser(resp.user);
        } else {
          setTimeout(() => {
  ...
  return (
    <div
      onClick={
        method === "popup" ? doSignInWithGoogle : doSignInWithGoogleRedirect
      }
      className={className}
    >
      {children ?? (
...
export function GithubSignInButton({
  ...
  function GithubLogo({ height = 24, width = 24, ...props }) {
    return (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width={width}
  ...
  return (
    <div onClick={doSignInWithGithub} className={className}>
      {children ?? (
        <button className="w-full bg-white text-gray-500 font-medium rounded-lg p-2 text-sm border shadow-sm hover:shadow-md transition-all flex flex-row items-center justify-center gap-2">
...
export function EmailSignInButton({
  ...
  return (
    <div onClick={doSignInWithEmailAndPassword} className={className}>
      {children}
    </div>
  );
}
...
export function EmailSignUpButton({
  ...
  return (
    <div onClick={doCreateUserWithEmailAndPassword} className={className}>
      {children}
    </div>
  );
}
...
export function ProfileButton({
  ...
  return (
    <Popover
      ...
      content={<ProfilePopup user={currentUser} />}
...
function LogoutLogo({ height = 20, width = 20, ...props }) {
  return (
    ...
    >
      <g>
        <path d="M7 6a1 1 0 0 0 0-2H5a1 1 0 0 0-1 1v14a1 1 0 0 0 1 1h2a1 1 0 0 0 0-2H6V6zm13.82 5.42-2.82-4a1 1 0 0 0-1.39-.24 1 1 0 0 0-.24 1.4L18.09 11H10a1 1 0 0 0 0 2h8l-1.8 2.4a1 1 0 0 0 .2 1.4 1 1 0 0 0 .6.2 1 1 0 0 0 .8-.4l3-4a1 1 0 0 0 .02-1.18z"></path>
      </g>
    </svg>
  );
}

function ProfileButtonTrigger({
  user,
  size,
}: {
  user: User | null;
 ...
 * 
 */
export function GoogleSignInButton({ children, className, method = "popup" }: { children?: React.ReactNode, className?: string, method?: "popup" | "redirect" }) {
    const doSignInWithGoogle = async () => {
        const provider = new GoogleAuthProvider();
        const resp = await signInWithPopup(auth, provider);
        if (resp) {
    ...
    return <div onClick={method == "popup" ? doSignInWithGoogle : doSignInWithGoogleRedirect} className={className}>
        {
            children ??
            <button className="w-full bg-white text-gray-500 font-medium rounded-lg p-2 text-sm border shadow-sm hover:shadow-md transition-all flex flex-row items-center justify-center gap-2">
                <GoogleLogo height={18} width={18} />
...
export function GithubSignInButton({ children, className }: { children?: React.ReactNode, className?: string }) {
    const doSignInWithGithub = async () => {
        const provider = new GithubAuthProvider();
        console.log("Provider : ", provider)
        const resp = await signInWithPopup(auth, provider);
    ...
    function GithubLogo({ height = 24, width = 24, ...props }) {
        return <svg xmlns="http://www.w3.org/2000/svg" width={width} height={height} viewBox="0 0 48 48">
    ...
    return <div onClick={doSignInWithGithub} className={className}>
        {
            children ??
            <button className="w-full bg-white text-gray-500 font-medium rounded-lg p-2 text-sm border shadow-sm hover:shadow-md transition-all flex flex-row items-center justify-center gap-2">
                <GithubLogo height={18} width={18} />
